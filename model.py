...
def precompute_theta_pos_frequencies(head_dim: int, seq_len: int, device: str, theta: float = 10000.0):
    # As written in the paragraph 3.2.2 of the paper, the dimention of the embedding should be an even number
...
class SelfAttention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.wq = nn.Linear(args.dim, args.n_heads * self.head_dim, bias = False)    # W_q
        self.wk = nn.Linear(args.dim, self.n_kv_heads * self.head_dim, bias = False) # W_k
        self.wv = nn.Linear(args.dim, self.n_kv_heads * self.head_dim, bias = False) # W_v
        self.wo = nn.Linear(args.n_heads * self.head_dim, args.dim, bias = False)    # W_o
...
class Transformer(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        assert args.vocab_size != -1, "Vocabulary size not set"
        ...
        # The Encoder block is added 'n' times
        for layer_id in range(args.n_layers):
            self.layers.append(EncoderBlock(args))

        # Normallizaton and Linearization
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        # Due to KV cache, we need to only give one token at a given time
        # (batch, Seq_Len)
